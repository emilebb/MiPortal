...
async function processDispatch(configured, dispatch) {
  ...
  if (env.NEWSLETTER_SEND_ENABLED !== 'true') {
    ...
    if (validEmail(testEmail)) {
      ...
      if (outcome.ok) {
        log(env, `Modo prueba: novedad enviada al correo de prueba (publicación ${resourceId})`);
      } else {
        log(env, `Modo prueba: el envío al correo de prueba falló (${outcome.error.status} ${outcome.error.detail})`);
      }
      await completeDispatch(configured, dispatch.id, 'skipped', 0, 0, 0,
        outcome.ok ? null : `falló el envío de prueba (${outcome.error.status})`);
    } else {
      log(env, 'Modo prueba: NEWSLETTER_SEND_ENABLED no está en true y no hay NEWSLETTER_TEST_EMAIL; nada enviado');
